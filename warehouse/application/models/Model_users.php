...
class Model_users extends CI_model{
    function cek_login($username,$password,$table){
        return $this->db->query("SELECT * FROM $table t, mu_jabatan j where j.id_jabatan = t.id_jabatan AND t.username='".$this->db->escape_str($username)."' AND t.password='".$this->db->escape_str($password)."'");
    }
